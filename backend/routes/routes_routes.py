...
def _closest_index(points: list[tuple[float, float]], lat: float, lon: float, step: int = 10):
    if not points:
        return 0
    ...
    for i in range(0, len(points), step):
        d = haversine_m(lat, lon, float(points[i][0]), float(points[i][1]))
        if d < best_d:
            best_d = d
            best_i = i

    start = max(0, best_i - step)
    end = min(len(points) - 1, best_i + step)
...
def _segment_distance_km(points: list[list[float]]):
    if len(points) < 2:
        return 0.0
    total = 0.0
...
@routes_bp.get("/<int:route_id>/via-cultural-item")
def route_via_cultural_item(route_id: int):
    ...
    try:
        ...
        item_lat = float(item_row[0])
        item_lon = float(item_row[1])
        ...
        idx_start = _closest_index(points, start_lat, start_lon, step=step)
        idx_item = _closest_index(points, item_lat, item_lon, step=step)

        if idx_start <= idx_item:
            segment = points[idx_start:idx_item + 1]
        else:
            segment = list(reversed(points[idx_item:idx_start + 1]))

        seg_points = [[float(lat), float(lon)] for lat, lon in segment]

        start_point = seg_points[0]
        end_point = seg_points[-1]

        leg1 = _osrm_walking_route(start_lat, start_lon, start_point[0], start_point[1])
        leg2 = _osrm_walking_route(end_point[0], end_point[1], item_lat, item_lon)

        polyline = []
        polyline.extend(leg1["polyline"])

        if polyline and seg_points:
            if polyline[-1] == seg_points[0]:
                polyline.extend(seg_points[1:])
            else:
                polyline.extend(seg_points)
        else:
            polyline.extend(seg_points)

        if polyline and leg2["polyline"]:
            if polyline[-1] == leg2["polyline"][0]:
                polyline.extend(leg2["polyline"][1:])
            else:
                polyline.extend(leg2["polyline"])
        else:
            polyline.extend(leg2["polyline"])
